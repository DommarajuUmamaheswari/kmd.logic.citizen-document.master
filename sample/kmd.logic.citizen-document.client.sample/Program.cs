...
using kmd.logic.citizen_document.client;
using kmd.logic.citizen_document.client.Models;
using kmd.logic.citizen_document.client.sample;
...
#pragma warning restore CA1031 // Do not catch general exception types
        ...
        private static async Task Run(AppConfiguration configuration)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                var citizenDocumentClient = new CitizenDocumentClient(httpClient, tokenProviderFactory, configuration.Citizen);
                var uploadDocument = await citizenDocumentClient.UploadAttachmentWithHttpMessagesAsync(new Guid(configuration.SubscriptionId) , configuration.ConfiguartionId, configuration.RetentionPeriodInDays, configuration.Cpr, configuration.DocumentType, configuration.Document).ConfigureAwait(false);
